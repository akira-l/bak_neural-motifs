...
class VG(Dataset):
    def __init__(self, mode, roidb_file=VG_SGG_FN, dict_file=VG_SGG_DICT_FN,
        ...
        change_mode = ['val', 'train', 'test']
        data_dict = {}
        for cur_mode in change_mode:
            # Initialize
            self.mode = cur_mode
            ...
            pdb.set_trace()
            data_dict['ind_to_classes'] = self.ind_to_classes
            data_dict['ind_to_predicates'] = self.ind_to_predicates
            data_dict['filenames'] = self.filenames
            data_dict['gt_boxes'] = self.gt_boxes
            data_dict['gt_classes'] = self.gt_classes
            data_dict['relationships'] = self.relationships
            torch.save(data_dict, cur_mode+'-save-150vg-data_dict.pt')
...
def load_graphs(graphs_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    gt_classes = []
    relationships = []
    selected_rela = [4, 9, 10, 13, 15, 18, 26, 27, 28, 32, 34, 35, 36, 37, 39]
    for i in range(len(image_index)):
        ...
        if filter_non_overlap:
            ...
            if inc.size > 0:
                ...
                continue
            #selected [4, 9, 10, 13, 15, 18, 26, 27, 28, 32, 34, 35, 36, 37, 39]


        '''
        rels_tmp_label = rels[:, 2]
        rels_select_mask = rels_tmp_label == selected_rela[0]
        for select_num in range(1, len(selected_rela)):
            rels_select_mask += (rels_tmp_label == selected_rela[select_num])
        rels = rels[np.where(rels_select_mask)[0]]
        if rels_select_mask.all() == False:
            split_mask[image_index[i]] = 0
        else:
            split_mask[image_index[i]] = 2
        '''
