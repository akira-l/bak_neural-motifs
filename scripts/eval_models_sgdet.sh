...
if [ $1 == "0" ]; then
    ...
    -ckpt checkpoints/stanford-sgdet/vgrel-9.tar -cache stanford_sgdet.pkl -test

elif [ $1 == "2" ]; then
    echo "EVALING MOTIFNET"
    python models/eval_rels.py -m sgdet -model motifnet -order leftright -multipred -nl_obj 2 -nl_edge 4 -b 1 -clip 5 \
        -p 100 -hidden_dim 512 -pooling_dim 4096 -lr 1e-3 -ngpu 1 -test -ckpt checkpoints/motifnet-sgdet/vgrel-motifnet-sgdet.tar -nepoch 50 -cache motifnet_sgdet.pkl -use_bias 2>&1 | tee sgdet_sgdet.log
fi
